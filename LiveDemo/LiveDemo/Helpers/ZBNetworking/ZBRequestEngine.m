...
- (instancetype)init {
    self = [super init];
    if (self) {
        //Unconditionally trust certificates returned by the server。
        self.securityPolicy = [AFSecurityPolicy policyWithPinningMode:AFSSLPinningModeNone];
        self.securityPolicy = [AFSecurityPolicy defaultPolicy];
        self.securityPolicy.allowInvalidCertificates = YES;
        self.securityPolicy.validatesDomainName = NO;
        /*Because the data returned by the cache interworking server must be binary*/
...
//    NSLog(@"======== self.responseSerializer.acceptableContentTypes  ========%@", self.responseSerializer.acceptableContentTypes);
